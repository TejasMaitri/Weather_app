...
def fetch_weather_details(city_name):
    url = f"http://api.openweathermap.org/data/2.5/weather?q={
        city_name}&appid={openweathermap_api_key}"
...
def get_openweathermap_current_weather(city):
    url = f"http://api.openweathermap.org/data/2.5/weather?q={
        urllib.parse.quote(city)}&appid={openweathermap_api_key}&units=metric"
    response = requests.get(url)
...
def main():
    # Set the title of the web app tab
    st.set_page_config(page_title="Weather Forecast")
    st.title("Weather Forecast App")  # Set the title of the web app
        ...
        if weather_details:
            ...
            with col1:
                ...
                st.write(f"Weather Condition: {
                    weather_details['Weather']['Condition']}")
                st.write(f"Weather Description: {
                    weather_details['Weather']['Description']}")
                st.write(f"Temperature: {
                    weather_details['Main']['Temperature']:.2f} °C")
                st.write(f"Feels Like: {
                    weather_details['Main']['Feels Like']:.2f} °C")
            with col2:

                st.write(f"Pressure: {
                         weather_details['Main']['Pressure']} hPa")
                ...
                st.write(f"Wind Direction: {
                    weather_details['Wind']['Direction']}°")
    ...
    if st.button("Predictions For Next 3 Days"):
        ...
        if response.status_code == 200:
            ...
            models = {var: RandomForestRegressor(
                n_estimators=100) for var in target_variables}
            ...
            for day in range(2, num_days + 1):
                ...
                with col1:
                    st.write(
                        f"Temp: {round(models['temp'].predict(latest_data)[0], 2)} °C")
                    st.write(f"Feels Like: {
                             round(models['feelslike'].predict(latest_data)[0], 2)} °C")
                    st.write(f"Humidity: {
                             round(models['humidity'].predict(latest_data)[0], 2)}%")
                    st.write(f"Wind Speed: {
                             round(models['windspeed'].predict(latest_data)[0], 2)} m/s")
                with col2:
                    st.write(f"Cloud Cover: {
                             round(models['cloudcover'].predict(latest_data)[0], 2)}%")
                    st.write(f"Precipitation: {
                             round(models['precip'].predict(latest_data)[0], 2)} mm")
                    st.write(
                        f"Dew: {round(models['dew'].predict(latest_data)[0], 2)} °C")
                    st.write(f"Solar Radiation: {
                             round(models['solarradiation'].predict(latest_data)[0], 2)} W/m^2")
                st.write("---")
        else:
            st.error("Failed to fetch data. Status code: " +
